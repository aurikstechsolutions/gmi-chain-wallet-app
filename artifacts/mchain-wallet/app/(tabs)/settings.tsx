...
export default function SettingsScreen() {
  ...
  const [pendingKeyAction, setPendingKeyAction] = useState<"reveal" | "export" | null>(null);
  ...
  function requestKeyPin(action: "reveal" | "export") {
    void requestPin({
      title: action === "reveal" ? "Reveal Private Key" : "Export Private Key",
      subtitle: action === "reveal"
        ? "Enter your PIN to reveal the key. Never share it with anyone."
        : "Enter your PIN to copy the key. Make sure no one can see your screen.",
      onSuccess: () => {
        if (action === "reveal") void revealPrivateKey();
        else void exportPrivateKey();
      },
      onCancel: () => {},
    });
  }

  async function authorizeKeyAction(action: "reveal" | "export") {
    if (!(await hasPin())) {
