...
import { GMI_CHAIN_ID, GMI_NATIVE_SYMBOL } from "@/services/chain";
import {
...
import {
  BSC_NATIVE_DECIMALS,
  fetchBscBalanceRaw,
  fetchBscTokenBalanceRaw,
  formatBscAmount,
  isValidBscAddress,
  parseBscAmount,
  quoteBscTransfer,
  sendQuotedBscTransaction,
  type BscTransferQuote,
} from "@/services/bsc";
...
type SelectedAsset =
  | { kind: "native"; chain: "gmi" | "bsc" | "solana" }
  | { kind: "token"; token: CustomToken };

const SOL_LOGO = require("../../assets/images/solana-coingecko.png");
const BNB_LOGO = require("../../assets/images/bnb-coingecko.png");
...
function AssetPickerSheet({
  ...
  const isGmiSelected = selected.kind === "native" && selected.chain === "gmi";
  const isBscSelected = selected.kind === "native" && selected.chain === "bsc";
  const isSolSelected = selected.kind === "native" && selected.chain === "solana";
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose} statusBarTranslucent>
      <View style={s.overlay}>
        ...
        <View style={s.sheet}>
          ...
          <TouchableOpacity
            style={s.row}
            onPress={() => { onSelect({ kind: "native", chain: "gmi" }); onClose(); }}
            activeOpacity={0.75}
          >
            <View style={s.iconWrap}>
              <Text style={s.iconText}>{GMI_NATIVE_SYMBOL}</Text>
            </View>
            <View style={s.info}>
              <Text style={s.name}>GMI Chain</Text>
              <Text style={s.sub}>{GMI_NATIVE_SYMBOL} · Native Coin</Text>
            </View>
            {isGmiSelected && (
          ...
          <TouchableOpacity
            style={s.row}
            onPress={() => { onSelect({ kind: "native", chain: "bsc" }); onClose(); }}
            activeOpacity={0.75}
          >
            <Image source={BNB_LOGO} style={s.tokenImg} />
            <View style={s.info}>
              <Text style={s.name}>BNB Smart Chain</Text>
              <Text style={s.sub}>BNB · Native Coin</Text>
            </View>
            {isBscSelected && (
              <View style={s.checkWrap}>
                <Icon name="checkmark" size={14} color="#FFF" />
          ...
          <TouchableOpacity
            style={s.row}
            onPress={() => { onSelect({ kind: "native", chain: "solana" }); onClose(); }}
            activeOpacity={0.75}
          >
            <Image source={SOL_LOGO} style={s.tokenImg} />
            <View style={s.info}>
              <Text style={s.name}>Solana</Text>
              <Text style={s.sub}>SOL · Native Coin</Text>
            </View>
            {isSolSelected && (
              <View style={s.checkWrap}>
                <Icon name="checkmark" size={14} color="#FFF" />
          ...
          {[...defaultTokens, ...tokens].map((token) => {
            ...
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.75}
              >
                {token.symbol.toUpperCase().includes("USDT") ? (
                  <UsdtLogo />
                ) : token.logoUrl ? (
...
export default function SendScreen() {
  ...
  const { mxcAddress, ethAddress, solAddress, getPrivateKey, activeWallet } = useWallet();
  ...
  const [selectedAsset, setSelectedAsset] = useState<SelectedAsset>({ kind: "native", chain: "gmi" });
  const [showAssetPicker, setShowAssetPicker] = useState(false);
  const [quoting, setQuoting] = useState(false);
  const [bscQuote, setBscQuote] = useState<BscTransferQuote | null>(null);
  ...
  // Contract-backed assets on each supported network can be selected as token transfers.
  const defaultTokens = DEFAULT_ASSETS
    .filter((a) => (a.chain === "gmi" || a.chain === "bsc" || a.chain === "solana") && !!a.contractAddress)
    .map(defaultAssetToCustomToken);

  const isToken = selectedAsset.kind === "token";
  const selectedToken = isToken ? selectedAsset.token : null;
  const assetChain = isToken ? (selectedToken?.chain ?? "gmi") : selectedAsset.chain;
  const isSolana = assetChain === "solana";
  const isBsc = assetChain === "bsc";
  ...
  const { data: tokenBalRaw } = useQuery({
    ...
    enabled: !!selectedToken && !isSolana && !isBsc && !!account?.ethAddress,
    staleTime: 15_000,
    refetchInterval: 30_000,
  });

  const { data: solBalanceRawText = "0" } = useQuery({
    queryKey: ["solanaBalance", solAddress],
    queryFn: async () => (await fetchSolanaBalanceRaw(solAddress!)).toString(),
    enabled: !!solAddress,
    staleTime: 15_000,
    refetchInterval: 30_000,
  });

  const { data: solTokenBalanceRawText } = useQuery({
    queryKey: ["solanaTokenBalance", selectedToken?.contractAddress, solAddress],
    queryFn: async () => (
      await fetchSolanaTokenBalanceRaw(selectedToken!.contractAddress, solAddress!)
    ).toString(),
    enabled: !!selectedToken && isSolana && !!solAddress,
    staleTime: 15_000,
    refetchInterval: 30_000,
  });

  const { data: bscBalanceRawText = "0" } = useQuery({
    queryKey: ["bscBalanceRaw", ethAddress],
    queryFn: async () => (await fetchBscBalanceRaw(ethAddress!)).toString(),
    enabled: !!ethAddress,
    staleTime: 15_000,
    refetchInterval: 30_000,
  });

  const { data: bscTokenBalanceRawText } = useQuery({
    queryKey: ["bscTokenBalanceRaw", selectedToken?.contractAddress, ethAddress],
    queryFn: async () => (
      await fetchBscTokenBalanceRaw(selectedToken!.contractAddress, ethAddress!)
    ).toString(),
    enabled: !!selectedToken && isBsc && !!ethAddress,
    staleTime: 15_000,
    refetchInterval: 30_000,
  });

  const symbol = isToken ? selectedToken!.symbol : isSolana ? "SOL" : isBsc ? "BNB" : GMI_NATIVE_SYMBOL;
  const mcBalance = weiToMc(account?.balance ?? "0");
  const mcBalanceNum = parseFloat(mcBalance.replace(/,/g, ""));
  const solBalanceRaw = BigInt(solBalanceRawText);
  const solTokenBalanceRaw = solTokenBalanceRawText === undefined
    ? undefined
    : BigInt(solTokenBalanceRawText);
  const solBalance = formatSolanaAmount(solBalanceRaw, SOLANA_NATIVE_DECIMALS);
  const bscBalanceRaw = BigInt(bscBalanceRawText);
  const bscTokenBalanceRaw = bscTokenBalanceRawText === undefined
    ? undefined
    : BigInt(bscTokenBalanceRawText);
  const bscBalance = formatBscAmount(bscBalanceRaw, BSC_NATIVE_DECIMALS);
  const tokenBalanceNum = isSolana
    ? Number(solTokenBalanceRaw ?? 0n) / Math.pow(10, selectedToken?.decimals ?? 0)
    : isBsc
      ? Number(bscTokenBalanceRaw ?? 0n) / Math.pow(10, selectedToken?.decimals ?? 0)
    : tokenBalRaw !== undefined && selectedToken
  ...
  const SPL_TOKEN_FEE_SOL = 0.003;
  const TX_FEE_LAMPORTS = 10_000n;
  const SPL_TOKEN_FEE_LAMPORTS = 3_000_000n;
  const BSC_NATIVE_FEE_RESERVE_WEI = 100_000_000_000_000n;
  ...
  function validateInput(): string | null {
    ...
    if (isSolana) {
      if (!isValidSolanaAddress(trimmed)) return "Enter a valid Solana address";
      if (trimmed === solAddress) return "Cannot send to your own address";
      let amountRaw: bigint;
      try {
        amountRaw = parseSolanaAmount(
          amount,
          isToken ? selectedToken!.decimals : SOLANA_NATIVE_DECIMALS,
        );
      } catch (error) {
        return error instanceof Error ? error.message : "Enter a valid amount";
      }
      if (isToken) {
        if (solTokenBalanceRaw === undefined) return "Loading token balance…";
        if (amountRaw > solTokenBalanceRaw) return `Insufficient ${symbol} balance`;
        if (solBalanceRaw < SPL_TOKEN_FEE_LAMPORTS) {
          return `Need at least ${SPL_TOKEN_FEE_SOL} SOL for network and token-account fees`;
        }
      } else {
        if (solBalanceRaw === 0n) return "Your wallet has no SOL balance";
        if (amountRaw + TX_FEE_LAMPORTS > solBalanceRaw) {
          return "Insufficient SOL balance for amount + fee";
        }
      }
      return null;
    }
    if (isBsc) {
      if (!isValidBscAddress(trimmed)) return "Enter a valid BNB Smart Chain address";
      if (trimmed.toLowerCase() === ethAddress?.toLowerCase()) return "Cannot send to your own address";
      let amountRaw: bigint;
      try {
        amountRaw = parseBscAmount(
          amount,
          isToken ? selectedToken!.decimals : BSC_NATIVE_DECIMALS,
        );
      } catch (error) {
        return error instanceof Error ? error.message : "Enter a valid amount";
      }
      if (isToken) {
        if (bscTokenBalanceRaw === undefined) return "Loading token balance…";
        if (amountRaw > bscTokenBalanceRaw) return `Insufficient ${symbol} balance`;
        if (bscBalanceRaw === 0n) return "Your wallet has no BNB for the network fee";
      } else {
        if (bscBalanceRaw === 0n) return "Your wallet has no BNB balance";
        if (amountRaw > bscBalanceRaw) return "Insufficient BNB balance";
      }
      return null;
    }
    const isValidGmi = trimmed.startsWith("gmi1") && trimmed.length >= 20;
    const isValidEth = /^0x[0-9a-fA-F]{40}$/.test(trimmed);
    if (!isValidGmi && !isValidEth) return "Enter a valid gmi1... or 0x... address";
    const toEth  = isValidGmi ? mxcAddressToEthAddress(trimmed).toLowerCase() : trimmed.toLowerCase();
    const myEth  = mxcAddress?.startsWith("gmi1") ? mxcAddressToEthAddress(mxcAddress).toLowerCase() : (mxcAddress ?? "").toLowerCase();
    ...
    if (isToken) {
      ...
      if (mcBalanceNum < TX_FEE_MC) return `Need at least ${TX_FEE_MC} ${GMI_NATIVE_SYMBOL} in your wallet for the gas fee`;
    } else {
      if (mcBalanceNum === 0) return "Your wallet has no balance on this network";
      if (amt + TX_FEE_MC > mcBalanceNum) return `Insufficient balance — need ${(amt + TX_FEE_MC).toFixed(4)} ${GMI_NATIVE_SYMBOL} (amount + fee)`;
    }
    return null;
  }
  ...
  async function handleContinue() {
    ...
    if (isBsc) {
      if (!ethAddress) {
        setError("BNB Smart Chain account is not available for this wallet");
        return;
      }
      setQuoting(true);
      try {
        const quote = await quoteBscTransfer({
        ...
        setError(quoteError instanceof Error ? quoteError.message : "Could not estimate the BNB Smart Chain fee");
        return;
      } finally {
        setQuoting(false);
      }
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (isBsc) {
        if (!ethAddress) throw new Error("BNB Smart Chain account is not available for this wallet");
        if (!bscQuote) throw new Error("BNB Smart Chain fee quote is missing. Please review again.");
        const hash = await sendQuotedBscTransaction(privateKey, bscQuote);
        result = { txHash: hash };
      } else if (isSolana) {
        if (!solAddress) throw new Error("Solana account is not available for this wallet");
        const signature = isToken && selectedToken
          ...
          : await sendSol(privateKey, recipient.trim(), amount);
        result = { txHash: signature };
      } else if (isToken && selectedToken) {
      ...
      if (isToken && selectedToken) {
        if (isSolana) {
          qc.invalidateQueries({ queryKey: ["solanaTokenBalance", selectedToken.contractAddress, solAddress] });
        } else if (isBsc) {
          qc.invalidateQueries({ queryKey: ["bscTokenBalanceRaw", selectedToken.contractAddress, ethAddress] });
        } else {
      ...
      if (isSolana) {
        qc.invalidateQueries({ queryKey: ["solanaBalance", solAddress] });
      } else if (isBsc) {
        qc.invalidateQueries({ queryKey: ["bscBalanceRaw", ethAddress] });
      }
      slideToStep("success");
  ...
  async function handlePasteAddress() {
    ...
    if (
      (isSolana && isValidSolanaAddress(trimmed)) ||
      (isBsc && isValidBscAddress(trimmed)) ||
      (!isSolana && !isBsc && (trimmed.startsWith("gmi1") || /^0x[0-9a-fA-F]{40}$/.test(trimmed)))
    ) {
  ...
  function setAmountPct(pct: number) {
    if (isSolana) {
      const balanceRaw = isToken ? (solTokenBalanceRaw ?? 0n) : solBalanceRaw;
      const feeRaw = isToken ? 0n : TX_FEE_LAMPORTS;
      const availableRaw = balanceRaw > feeRaw ? balanceRaw - feeRaw : 0n;
      const amountRaw = availableRaw * BigInt(Math.round(pct * 100)) / 100n;
      setAmount(formatSolanaAmount(
        amountRaw,
        isToken ? selectedToken!.decimals : SOLANA_NATIVE_DECIMALS,
      ));
      return;
    ...
    if (isBsc) {
      const balanceRaw = isToken ? (bscTokenBalanceRaw ?? 0n) : bscBalanceRaw;
      const feeRaw = isToken ? 0n : BSC_NATIVE_FEE_RESERVE_WEI;
      const availableRaw = balanceRaw > feeRaw ? balanceRaw - feeRaw : 0n;
      const amountRaw = availableRaw * BigInt(Math.round(pct * 100)) / 100n;
      setAmount(formatBscAmount(
        amountRaw,
        isToken ? selectedToken!.decimals : BSC_NATIVE_DECIMALS,
      ));
      return;
  ...
  function fullReset() {
    ...
    setBscQuote(null); setQuoting(false);
    setRecipientFocused(false); setAmountFocused(false);
    setLoading(false); setSelectedAsset({ kind: "native", chain: "gmi" }); setShowAssetPicker(false);
            ...
            <View style={s.confirmCard}>
              ...
              <View style={s.confirmRow}>
                ...
                <Text style={s.confirmValue}>
                  {isSolana
                    ? isToken ? `up to ~${SPL_TOKEN_FEE_SOL} SOL` : `~${TX_FEE_SOL} SOL`
                    : isBsc
                      ? `${formatBscAmount(BigInt(bscQuote?.feeWei ?? "0"), BSC_NATIVE_DECIMALS)} BNB`
                    : `~${TX_FEE_MC} ${GMI_NATIVE_SYMBOL}`}
                </Text>
              </View>
              {!isSolana && (
                <View style={[s.confirmRow, { borderBottomWidth: 0 }]}>
                  <Text style={s.confirmLabel}>Nonce</Text>
                  <Text style={s.confirmValue}>#{isBsc ? (bscQuote?.nonce ?? 0) : (account?.nonce ?? 0)}</Text>
                </View>
              )}
            </View>
  ...
  return (
    <View style={s.container}>
      <Animated.View style={{ flex: 1, transform: [{ translateX: slideAnim }] }}>
        ...
        >
          ...
          <TouchableOpacity style={s.assetSelector} onPress={() => setShowAssetPicker(true)} activeOpacity={0.8}>
            {!isToken && isSolana ? (
              <Image source={SOL_LOGO} style={s.assetSelectorImg} />
            ) : !isToken && isBsc ? (
              <Image source={BNB_LOGO} style={s.assetSelectorImg} />
            ) : isToken && selectedToken?.symbol.toUpperCase().includes("USDT") ? (
              <UsdtLogo />
            ...
            <View style={s.assetSelectorInfo}>
              ...
              <Text style={s.assetSelectorName}>
                {isToken
                  ? selectedToken!.name
                  : isSolana ? "Solana" : isBsc ? "BNB Smart Chain" : "GMI Chain"} ({symbol})
              </Text>
              <Text style={s.assetSelectorBal}>
                ...
                {isToken
                  ? isSolana
                    ...
                    : isBsc
                      ? `  ·  Gas: ${bscBalance} BNB`
                      : `  ·  Gas: ${mcBalance} ${GMI_NATIVE_SYMBOL}`
                  : ""}
              </Text>
            </View>
          ...
          <View style={s.fieldBlock}>
            ...
            <View style={[s.inputRow, recipientFocused && s.inputRowFocused]}>
              <TextInput
                style={s.textInput}
                placeholder={isSolana ? "Solana address" : isBsc ? "0x BNB Chain address" : "gmi1... or 0x..."}
          ...
          >
            <View  style={s.primaryGrad}>
              ...
              <Text style={s.primaryBtnText}>{quoting ? "Checking network fee…" : "Continue"}</Text>
            </View>
          </TouchableOpacity>
        </ScrollView>
      </Animated.View>
      ...
      <AssetPickerSheet
        ...
        selected={selectedAsset}
        onSelect={(a) => { setSelectedAsset(a); setAmount(""); setError(""); setBscQuote(null); }}
      />
      <Toast message={toast} visible={!!toast} onHide={() => setToast("")} />
      <QRScannerModal
        visible={showScanner}
        ...
        network={isSolana ? "solana" : isBsc ? "bsc" : "gmi"}
      />
    </View>
  );
}
