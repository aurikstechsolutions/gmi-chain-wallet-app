  ...
  const content = (
    <>
      {asset ? (
        ...
        <Text style={[styles.tokenSymbol, { color: colors.mutedForeground }]}>Select asset</Text>
      )}
...
export default function SwapScreen() {
  ...
  const fromToken = fromTokenId === "gmi" ? nativeToken : fromTokenId === "wusdt" ? wusdtToken : undefined;
  const toToken = toTokenId === "gmi" ? nativeToken : toTokenId === "wusdt" ? wusdtToken : undefined;
  const fromAsset: AssetOption | undefined = fromTokenId && fromToken
    ? { id: fromTokenId, symbol: fromToken.symbol, name: fromToken.name, networkLabel: "GMI Chain" }
    : undefined;
  const toAsset: AssetOption | undefined = toTokenId && toToken
    ? { id: toTokenId, symbol: toToken.symbol, name: toToken.name, networkLabel: "GMI Chain" }
    : undefined;
  ...
  function selectSwapAsset(side: "from" | "to", id: string) {
    if (side === "from") setFromTokenId(id);
    else setToTokenId(id);
    setSwapAmount("");
    resetAmmStatus();
    setSelector(null);
  }
  const selectedSelectorId = selector === "swap-from" ? (fromAsset?.id ?? "") : selector === "swap-to" ? (toAsset?.id ?? "") : bridgeSource;
  const selectorOptions = selector === "bridge-route"
    ? bridgeRouteOptions
    : swapSelectorOptions.map((option) => ({
        ...option,
        disabled: selector === "swap-from" ? option.id === toTokenId : option.id === fromTokenId,
      }));
              ...
              <View style={s.formCard}>
                ...
                {liquidityMode === "add" ? (
                  <>
                    ...
                    <View style={s.switchRow}><View style={s.divider} /><TouchableOpacity style={s.switchButton} onPress={() => { setFromTokenId(toTokenId); setToTokenId(fromTokenId); setSwapAmount(""); resetAmmStatus(); }} disabled={!fromTokenId || !toTokenId} activeOpacity={0.75} accessibilityRole="button" accessibilityLabel="Switch swap assets"><Icon name="swap-horizontal" size={17} color={fromTokenId && toTokenId ? colors.primary : colors.mutedForeground} /></TouchableOpacity><View style={s.divider} /></View>
                    ...
                    <View style={s.detailCard}>
                      <View style={s.detailRow}><Text style={s.detailLabel}>Rate</Text><Text style={s.detailValue}>{quote && fromAsset && toAsset ? `1 ${fromAsset.symbol} ≈ ${formatAmmUnits(quote.amountOut * 10n ** BigInt(fromToken?.decimals ?? 18) / parseAmmUnits(swapAmount, fromToken?.decimals ?? 18), toToken?.decimals ?? 18)} ${toAsset.symbol}` : "—"}</Text></View>
